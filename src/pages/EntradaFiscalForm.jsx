...
import { FIELDS, SECTIONS } from './entradasFiscaisFields.js'
...
export default function EntradaFiscalForm({ initial, submitting, onCancel, onSave }) {
    ...
    <div className="modal-backdrop" onMouseDown={(e) => {
      ...
      <form className="modal modal-xl" onSubmit={handleSubmit}>
        <div className="modal-head">
          <h2>{editing ? `Editar entrada #${initial.id}` : 'Nova entrada fiscal'}</h2>
          <p>Preencha os dados do item do documento fiscal de entrada.</p>
        </div>
            ...
            <fieldset key={section} className="form-section">
              ...
              <div className="form-grid">
                {FIELDS.filter(f => f.section === section).map((f, idx) => {
                  ...
                  return (
                    <div key={f.name} className={`field ${err ? 'has-error' : ''} ${f.multiline ? 'field-wide' : ''}`}>
                      ...
                      {f.multiline
                        ...
                        : <input
                            id={`f-${f.name}`}
                            ref={first ? firstRef : undefined}
                            type={f.kind === 'date' ? 'date' : (f.kind === 'numeric' ? 'text' : 'text')}
