...
const HEADER_ALIASES = {
  codigo: ['codigo', 'código', 'code', 'cod', 'codigogrupo'],
  descricao: ['descricao', 'descrição', 'description', 'descr', 'nome', 'name'],
  palavraChave: [
    'palavrachave', 'palavrachav', 'palavradechave',
    'keyword', 'chave', 'tag'
  ]
}
...
function validate(body) {
  ...
  if (!palavraChave) errors.palavraChave = 'Palavra-chave é obrigatória'
  else if (palavraChave.length > 100) errors.palavraChave = 'Palavra-chave deve ter até 100 caracteres'

  return { errors, codigo, descricao, palavraChave }
}
...
const COLS = 'id, codigo, descricao, palavra_chave, created_at, updated_at'
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (q) {
      params.push(`%${q}%`)
      where = `WHERE codigo ILIKE $1 OR descricao ILIKE $1 OR palavra_chave ILIKE $1`
    }
...
router.get('/template.xlsx', (_req, res, next) => {
  try {
    ...
    const ws = XLSX.utils.aoa_to_sheet([
      ['codigo', 'descricao', 'palavra_chave'],
    ...
    res.setHeader('Content-Disposition',
      'attachment; filename="grupos_produtos_modelo.xlsx"')
    res.send(buf)
  } catch (err) { next(err) }
})
  ...
  upload.single('file')(req, res, async (uerr) => {
    ...
    try {
      ...
      raw.forEach((rowRaw, i) => {
        ...
        const palavraChave = String(row.palavraChave ?? '').trim()

        if (!codigo && !descricao && !palavraChave) return
        ...
        if (!palavraChave) rowErrors.push('palavra_chave vazia')
        else if (palavraChave.length > 100) rowErrors.push('palavra_chave > 100 caracteres')
        ...
        valid.push({ rowNum, codigo, descricao, palavraChave })
      })
        ...
        for (const r of valid) {
          if (mode === 'upsert') {
            const result = await client.query(
              `INSERT INTO grupos_produtos (codigo, descricao, palavra_chave)
               VALUES ($1, $2, $3)
              ...
              [r.codigo, r.descricao, r.palavraChave]
            )
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `SELECT ${COLS} FROM grupos_produtos WHERE id = $1`,
      [id]
    )
...
router.post('/', async (req, res, next) => {
  try {
    const { errors, codigo, descricao, palavraChave } = validate(req.body)
    ...
    if (err.code === '23505') {
      return res.status(409).json({
        ...
        message: 'Já existe um grupo com esse código'
      })
    }
    next(err)
  }
  ...
  try {
    ...
    const { rows } = await query(
      `UPDATE grupos_produtos
       SET codigo = $1, descricao = $2, palavra_chave = $3, updated_at = NOW()
       WHERE id = $4
       RETURNING ${COLS}`,
      [codigo, descricao, palavraChave, id]
    )
    if (!rows.length) return res.status(404).json({ error: 'not_found' })
    res.json(mapRow(rows[0]))
  } catch (err) {
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    const { rowCount } = await query(`DELETE FROM grupos_produtos WHERE id = $1`, [id])
