...
const HEADER_ALIASES = {
  codigo: ['codigo', 'código', 'code', 'cod', 'codigofilial', 'codfilial'],
  descricao: ['descricao', 'descrição', 'description', 'descr', 'nome', 'name'],
  abreviatura: ['abreviatura', 'abrev', 'sigla', 'alias']
}
...
function validate(body) {
  ...
  const abreviatura = typeof body?.abreviatura === 'string' ? body.abreviatura.trim() : ''
  ...
  if (!abreviatura) errors.abreviatura = 'Abreviatura é obrigatória'
  else if (abreviatura.length > 20) errors.abreviatura = 'Abreviatura deve ter até 20 caracteres'

  return { errors, codigo, descricao, abreviatura }
}
...
const COLS = 'id, codigo, descricao, abreviatura, created_at, updated_at'
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (q) {
      params.push(`%${q}%`)
      where = `WHERE codigo ILIKE $1 OR descricao ILIKE $1 OR abreviatura ILIKE $1`
    }
  ...
  upload.single('file')(req, res, async (uerr) => {
    ...
    try {
      ...
      raw.forEach((rowRaw, i) => {
        ...
        const abreviatura = String(row.abreviatura ?? '').trim()

        if (!codigo && !descricao && !abreviatura) return
        ...
        if (!abreviatura) rowErrors.push('abreviatura vazia')
        else if (abreviatura.length > 20) rowErrors.push('abreviatura > 20 caracteres')
        ...
        valid.push({ rowNum, codigo, descricao, abreviatura })
      })
        ...
        for (const r of valid) {
          if (mode === 'upsert') {
            const result = await client.query(
              `INSERT INTO filiais (codigo, descricao, abreviatura)
               VALUES ($1, $2, $3)
              ...
              [r.codigo, r.descricao, r.abreviatura]
            )
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `SELECT ${COLS} FROM filiais WHERE id = $1`,
      [id]
    )
...
router.post('/', async (req, res, next) => {
  try {
    const { errors, codigo, descricao, abreviatura } = validate(req.body)
    ...
    if (err.code === '23505') {
      return res.status(409).json({
        ...
        message: 'Já existe uma filial com esse código'
      })
    }
    next(err)
  }
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `UPDATE filiais
       SET codigo = $1, descricao = $2, abreviatura = $3, updated_at = NOW()
       WHERE id = $4
       RETURNING ${COLS}`,
      [codigo, descricao, abreviatura, id]
    )
    if (!rows.length) return res.status(404).json({ error: 'not_found' })
    res.json(mapRow(rows[0]))
  } catch (err) {
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    const { rowCount } = await query(
      `DELETE FROM filiais WHERE id = $1`,
      [id]
    )
